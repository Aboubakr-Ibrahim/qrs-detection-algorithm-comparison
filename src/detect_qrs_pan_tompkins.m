...
x=double(ecg(:)); band=cfg.panTompkinsBandHz;
band(2)=min(band(2),0.90*fs/2);
[b,a]=butter(2,band/(fs/2),'bandpass'); z=filtfilt(b,a,x);
energy=[0;diff(z)].^2;
...
diagnostic=struct('enhanced',z,'score',integrated,'threshold',threshold,...
